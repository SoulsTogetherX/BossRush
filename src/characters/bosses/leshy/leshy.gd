...
func _ready() -> void:
	...
	_add_to_sequence(hide_self, 5.6);
...
func hide_self() -> void:
	...
	tw.tween_property(self_pos, "scale", Vector2(1.0, 1.0), 0.3);
...
func dash() -> void:
	$Sprite2D.material.set_shader_parameter("modulate", Color("#ffffff"));
	$hurt_box.toggle_hurtbox(false);
	$dash_timer.start();
